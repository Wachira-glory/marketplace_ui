...
//       <div className="w-full h-16 bg-black flex items-center justify-between px-16">
//         {/* Logo */}
//         <Image src="/images/quikklogo.svg" alt="Logo" width={50} height={50} />
...
//           <span className="text-[#6366F1] text-[24px]">Forge</span>
//           <span className="text-[24px]">Marketplace</span>
//           <Image src="/images/Quikk.png"  width={36} height={19}  alt="Quikk Logo"  />
//         </div>
...
//   {/* Background Image */}
//   <Image
//     src={slides[currentIndex].image} width={1200} height={448}
//     alt="Carousel"
//     className="object-cover rounded-lg t-[54px] " 
//   />
...
const slides = [
  ...
  },
  {
    id: 2,
    image: "/images/secondlandingpageimage.png",
    highlight1: "your business",
    highlight2: "seamless payments",
    description: "Fast and secure transactions for your business growth",
    buttonGradient: "linear-gradient(to right, #3CA55C, #B5AC49)",
    bgColor: "#FFFFFF",
  },
...
const BlackDiv = () => {
  const [currentIndex, setCurrentIndex] = useState<number>(0); // Always a number

  return (
    <div>
      {/* Top Black Bar */}
      <div className="w-full h-16 bg-black flex items-center justify-between px-16">
        <Image src="/images/quikklogo.svg" alt="Logo" width={50} height={50} priority />
      ...
      <div className="bg-[#F3F6FF] p-8 rounded-lg h-[564px] mx-auto relative">
        ...
        <div className="relative w-[1200px] h-[448px] mx-auto rounded-lg overflow-hidden" style={{ background: slides[currentIndex].bgColor }}>
          {/* Background Image */}
          <Image src={slides[currentIndex].image} width={1200} height={448} alt="Carousel" className="object-cover rounded-lg t-[54px]" />

          {/* Blue Overlay */}
          <div className="absolute inset-0" style={{ background: "linear-gradient(to right, #2B5876, #4E4376)", opacity: 0.9 }}></div>

          {/* Overlay Content */}
          <div className="absolute inset-0 flex flex-col justify-center items-center text-center text-white px-8">
            <h1 className="text-4xl md:text-5xl font-bold">
              Discover <span className="text-[#F6FA83]">{slides[currentIndex].highlight1}</span> services in{" "}
              <span className="text-[#FFBA08]">{slides[currentIndex].highlight2}</span>
            </h1>
            <p className="text-lg md:text-xl mt-6">{slides[currentIndex].description}</p>

            {/* Browse Collection Button */}
            <button className="flex items-center text-white px-6 py-2 rounded-lg mt-8" style={{ background: slides[currentIndex].buttonGradient }}>
              <CiSearch className="text-xl" />
              <span className="ml-2">Browse Collection</span>
            </button>
          </div>
        ...
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
          {slides.map((_, index) => (
            <div key={index} className={`w-[9px] h-[9px] rounded-full ${index === currentIndex ? "bg-[#6366F1]" : "bg-gray-300"}`}></div>
          ))}
        </div>
      </div>
    </div>
